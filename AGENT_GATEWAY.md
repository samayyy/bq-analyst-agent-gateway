...
| Runtime logs: `Failed to send request to https://...mtls.googleapis.com/...`; gateway log shows `CONNECT` to an IP literal like `hostname: 240.0.0.2:443` matched by `default_denied` | The client-side mTLS path (auto-enabled by Agent Identity certs) loses the hostname at the gateway, so no registry entry can ever match it. Set `GOOGLE_API_USE_CLIENT_CERTIFICATE=false` in `.env` (in `.env.example` since this was found) and redeploy with `--agent_engine_id` — the SDKs then use plain hostnames, which Phase 1 registration covers. The gateway terminates mTLS and uses IAP/DPoP for agent auth, so client certs are not the protection mechanism on this path. |
